---
layout: chapter
title: "On te paie ce qui se vérifie"
part: "La référence"
order: 1007
metadata:
  principle: "10.07"
  reading_time_in_minutes: 3
categories:
  - visibilite
  - carriere
  - trace
seo:
  description: "Un prix n'est pas une mesure de ce que tu vaux. C'est un pari qu'un acheteur fait sur la partie de ton travail qu'il peut vérifier avant de signer."
  keywords: "build here, carriere, visibilite, builder, valeur, salaire, prix"
redirect_from:
  - /chapters/15-06-on-te-paie-ce-qui-se-verifie.html
---

## Le réflexe

> "Je veux être payé à ma valeur."

Personne ne conteste cette phrase, et c'est ce qui la rend inutilisable. Elle suppose un nombre juste, connu de toi, que quelqu'un en face refuse de payer.

## Le réflexe builder

> "Ma valeur, c'est ce que je sais faire tourner. Mon prix, c'est la partie que quelqu'un peut vérifier."

Deux choses différentes. Une seule des deux se négocie ce mois-ci.

## Pourquoi

Je réponds depuis des années "connais ta valeur, ne travaille pas gratuitement", et j'entends la phrase revenir traduite en "demande plus". C'est la mauvaise moitié, et la traduction est de ma faute. Reconnais d'abord ce qui est vrai. L'écart de salaire pour le même travail se compte en multiples, il ne récompense aucune compétence, et il rend l'offre venue d'ailleurs parfaitement rationnelle. Personne ne doit un vœu de pauvreté à personne, et celui qui répond à cette question par de la vocation est en général en train de vendre quelque chose.

Un prix n'est pas une mesure de ce que tu vaux. C'est un pari qu'un acheteur fait sur la partie de ton travail qu'il peut vérifier avant de signer. Il a quarante minutes et deux onglets ouverts. S'il ne trouve rien à ouvrir, il reste un seul élément vérifiable, le nom de celui qui te recommande, et le salaire dépend de qui te connaît plutôt que de ce que tu as fait.

Deux chemins mènent à une augmentation, devenir meilleur et se rendre visible, et le second est plus court sur ce marché. Celui qui vise seulement un salaire plus élevé prend le chemin le plus court. Il peut l'obtenir, mais ce salaire dépend alors d'une notoriété qu'il doit entretenir chaque année. Dans l'autre ordre, la liste de ce que tu sais faire tourner s'allonge, la partie vérifiable s'allonge derrière elle, et le chiffre suit. Il reste en deçà de ta valeur et progresse avec du retard. C'est le seul ordre qui ne dépende pas de qui est dans la salle.

La phrase se détourne dans les deux sens. Un employeur s'en sert pour rappeler que l'argent n'est pas l'essentiel, ce qui n'est vrai que dans la bouche de celui qui ne le verse pas. Un builder s'en sert comme slogan de négociation sans rien derrière, et une valeur qu'on annonce sans pouvoir la montrer est juste un prix plus élevé. Ne travaille pas gratuitement reste juste, pour une raison qui n'est pas la tienne. Un travail livré à zéro apprend à l'acheteur ce que ce travail coûte, et il l'applique à celui qui passe après toi.

## À essayer

Dix lignes, dix minutes. Pas un CV, pas des intitulés de poste.

> Ce que j'ai fait tourner, à quelle échelle, et ce qui a cassé dedans.

Marque ensuite les lignes qu'une personne qui ne te connaît pas peut vérifier ce soir sans te le demander. Ce sont ces lignes qui soutiennent ta négociation aujourd'hui. Le reste est ta valeur, et personne ne l'a encore écrit.

## Depuis ton siège

- **Product** : un acheteur a quarante minutes et deux onglets. C'est tout le budget de vérification.
- **Design** : un portfolio montre des images. Ce qui se vérifie, c'est ce que les images ont changé.
- **Founder** : un travail livré à zéro apprend à l'acheteur ce qu'il coûte, et il l'applique au suivant.
- **Manager** : sans rien à ouvrir, le seul élément vérifiable est le nom de celui qui recommande.
- **Recrutement** : tu paies ce que tu sais vérifier. Élargis ça avant de baisser une offre.

## À discuter

La dernière fois que quelqu'un ici a été augmenté ou débauché, qu'est-ce qui était vérifiable de l'extérieur avant que ça arrive ?
