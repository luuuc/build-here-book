---
layout: chapter
title: "Parle du problème avant de parler de toi"
part: "Visibilité"
order: 63
metadata:
  principle: "49"
  reading_time_in_minutes: 2
categories:
  - visibilite
  - distribution
  - marketing
seo:
  description: "Personne ne s'intéresse à une entreprise avant d'avoir compris ce qu'elle change pour lui, donc c'est la séquence qui fait le travail."
  keywords: "build here, visibilite, tech afrique, builder, parle, probleme, avant, parler"
---

## Le réflexe

Le pitch ouvre sur l'entreprise. Fondée en, une équipe de, construite sur, et voici les modules.

Le lecteur, qui t'a rencontré il y a neuf secondes, n'a aucune raison de s'intéresser à quoi que ce soit de tout ça.

## Le réflexe builder

> "Décris leur journée assez précisément pour qu'ils le reconnaissent, ensuite dis ce qu'on vend."

## Pourquoi

Personne ne s'intéresse à une entreprise avant d'avoir compris ce qu'elle change pour lui, donc c'est la séquence qui fait le travail. Une description précise du problème donne au lecteur l'impression que tu étais dans son bureau. Ensuite il continue, en supposant que celui qui connaît le problème à ce point sait probablement quelque chose sur la réponse.

C'est la précision qui porte tout. "Optimisez vos opérations" ne bouge personne. "Chaque soir tu retapes les commandes WhatsApp de la journée dans un tableur, et tu sais déjà que deux sont fausses" bouge ceux qui font exactement ça, et eux seuls. Bon échange.

Les fondateurs attrapent le slide équipe à ce moment-là. La crédibilité ne se revendique pas, elle se démontre par la justesse de la description. Celui qui nomme le détail de ta pire heure a prouvé plus qu'un mur de logos.

Un détail légèrement gênant vaut mieux qu'une généralité élégante. Le premier ne s'invente pas depuis un bureau, et le lecteur qui a vécu la scène le sait en une ligne.

## À essayer

Ouvre ta page d'accueil. Lis la première phrase à voix haute.

Si elle parle de toi, remplace-la par une phrase qui décrit la journée de ton client, assez précise pour qu'il dise "c'est exactement ça".

Relis à voix haute. Si le niveau de détail te rend légèrement nerveux, c'est le bon.

## À discuter

Notre première phrase. Elle décrit la semaine de qui, et est-ce que cette personne la reconnaîtrait comme la sienne ?
